...
def _permission_grant_always_permission_state():
    return build_permission_state(
        sandbox_policy=WorkspaceWriteSandboxPolicy(),
        approval_policy=ApprovalPolicy(
            mode="on_escalation",
            by_kind={"permission_grant": "always"},
        ),
    )


def _permission_grant_never_permission_state():
    return build_permission_state(
        sandbox_policy=WorkspaceWriteSandboxPolicy(),
        approval_policy=ApprovalPolicy(
            mode="on_escalation",
            by_kind={"permission_grant": "never"},
        ),
    )


def _command_execution_always_permission_state():
    return build_permission_state(
        sandbox_policy=WorkspaceWriteSandboxPolicy(),
        approval_policy=ApprovalPolicy(
            mode="on_escalation",
            by_kind={"command_execution": "always"},
        ),
    )


def _command_execution_never_permission_state():
    return build_permission_state(
        sandbox_policy=WorkspaceWriteSandboxPolicy(),
        approval_policy=ApprovalPolicy(
            mode="on_escalation",
            by_kind={"command_execution": "never"},
        ),
    )
...
async def test_given_permission_grant_always_override_when_workspace_read_then_permission_grant_approval_is_requested(
    tmp_path,
) -> None:
    base_ctx = worker_ctx(tmp_path)
    note = base_ctx.deps.workspace_root / "note.txt"
    ...
    ctx = SimpleNamespace(
        deps=WorkspaceDeps(
            ...
            shell_family=detect_default_shell_family(),
            approval_requester=approval_requester,
            permission_state=_permission_grant_always_permission_state(),
            read_only_worker=base_ctx.deps.read_only_worker,
        ),
        tool_call_id="call-read-override",
        tool_name="read",
    )
    ...
    assert (
        requests[0].reason
        == "allow read: note.txt (approval policy: permission_grant=always)"
    )
...
async def test_given_command_execution_always_override_when_shell_runs_in_workspace_then_command_execution_approval_is_requested(
    tmp_path,
    monkeypatch,
) -> None:
    workspace_root = tmp_path / "workspace"
    ...
    ctx = _FakeShellContext(
        deps=WorkspaceDeps(
            ...
            sandbox_executor=_Executor(),
            approval_requester=approval_requester,
            permission_state=_command_execution_always_permission_state(),
        )
    )
...
async def test_given_permission_grant_never_override_when_non_workspace_read_then_policy_denied_without_executing(
    tmp_path,
) -> None:
    workspace_root = tmp_path / "workspace"
    workspace_root.mkdir()
    ...
    ctx = SimpleNamespace(
        deps=WorkspaceDeps(
            ...
            shell_family=detect_default_shell_family(),
            approval_requester=approval_requester,
            permission_state=_permission_grant_never_permission_state(),
            read_only_worker=worker,
        ),
        tool_call_id="call-read-never-override",
        tool_name="read",
    )
...
async def test_given_command_execution_never_override_when_shell_network_is_blocked_then_command_does_not_run(
    tmp_path,
    monkeypatch,
) -> None:
    workspace_root = tmp_path / "workspace"
    ...
    ctx = _FakeShellContext(
        deps=WorkspaceDeps(
            ...
            sandbox_executor=executor,
            approval_requester=approval_requester,
            permission_state=_command_execution_never_permission_state(),
        )
    )
