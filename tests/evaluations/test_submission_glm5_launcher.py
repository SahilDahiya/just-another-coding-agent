...
def _copy_launcher(tmp_path: Path) -> Path:
    ...
    # Copy sibling shell scripts sourced by the launcher (logfire_env.sh,
    # docker_env.sh, etc.) so the test sandbox is self-contained.
    ...
    for sh_file in real_scripts_dir.glob("*.sh"):
        if sh_file.name == SCRIPT_PATH.name:
            continue  # already copied as the launcher
