...
def test_harbor_agent_can_enable_code_mode_only_from_env(
    monkeypatch, tmp_path: Path
) -> None:
    for module_name in list(sys.modules):
        if module_name.startswith("evaluations.harbor.agent") or module_name.startswith(
    ...
    _install_fake_harbor_modules()
    monkeypatch.setenv("OPENAI_API_KEY", "openai-secret")
    monkeypatch.setenv("JACA_HARBOR_CODE_MODE", "only")
    monkeypatch.setenv("LOGFIRE_TOKEN", "logfire-secret")
    ...
    assert len(commands) == 1
    assert " --code-mode " in commands[0].command
    assert " --code-mode-only " in commands[0].command
    assert commands[0].env["JACA_HARBOR_CODE_MODE"] == "only"
