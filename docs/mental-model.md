...
- `session_compaction` -- records a durable compaction summary, the run
  boundary it summarizes through, an optional first retained run boundary, and
  an authoritative compaction checkpoint for later resume

Example:

```json
{"type":"session_header","version":9,"workspace_root":"/abs/path/to/workspace"}
...
{"type":"session_compaction","compaction_id":"cmp-1","summarized_through_run_id":"abc","first_kept_run_id":null,"checkpoint_through_run_id":"abc","checkpoint_messages":[...],"summary":{"current_objective":"ship the fix","established_facts":[],"user_preferences":[],"important_paths":[],"read_paths":[],"modified_paths":[],"open_questions":[],"unresolved_work":[]}}
```
...
Sessions persist both durable public contract events and native PydanticAI message history. They also persist the effective per-run thinking setting. These serve different purposes and neither can replace the other. Live `assistant_text_delta` events still exist on the RPC stream for UI rendering, but they are transport-only and are not written into the canonical session file.
Compaction entries stay durable session metadata, but they now also carry an authoritative compaction checkpoint: raw `checkpoint_messages` plus `checkpoint_through_run_id`. The durable `summary` remains structured continuity state and is rendered into ephemeral per-run instructions at resume time rather than replayed as a persisted system prompt.
