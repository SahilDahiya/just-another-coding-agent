...
{"type":"session_turn_context","run_id":"abc","model":"openai-responses:gpt-5.3-codex","thinking":"high","workspace_root":"/abs/path/to/workspace","shell_family":"posix","current_date":"2026-04-03","instructions":"..."}
...
Sessions may also carry a persisted `session_turn_context` entry for a completed run. That entry is not part of conversation memory. It is a backend-owned snapshot of how that run was framed: model, thinking, workspace root, shell family, current date, and the full canonical instruction payload. In the current first slice, the runtime still rebuilds and reinjects the full canonical instructions on the next run. The persisted turn-context entry exists as a durable baseline and invalidation point, not as a diff-based prompt-optimization path yet.
