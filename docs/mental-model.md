...
- `run.enqueue` -- queues one non-blank prompt for an already-active session-backed run, with `mode: "next" | "later"`; `next` is attached only after the current tool phase completes, before the next model round-trip in the active run, and `later` is drained after the current run ends. Multiple queued prompts in the same bucket are combined into one follow-up prompt in FIFO order using blank-line separation
...
- `shell` -- runs one command with the active shell family (`posix`, executed with Bash semantics, or `powershell`) with `cwd` set to workspace root, uses a backend-owned default timeout when one is not provided explicitly, returns `{"exit_code": 0, "output": str}` on success, and returns explicit tool error results for non-zero exits or timeouts
