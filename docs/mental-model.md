...
- `shell` -- runs one command with the active shell family (`posix` or `powershell`) with `cwd` set to workspace root, returns `{"exit_code": 0, "output": str}` on success and explicit tool error results for non-zero exits or timeouts; `defer=true` is the explicit contract for genuinely long shell/build/test work
...
When tracing is enabled, the runtime also emits canonical `jaca.run` and `jaca.tool_call` spans so evaluation-side watchdog helpers can detect long-tool and shell-heavy probe loops without inspecting session JSONL by hand.
