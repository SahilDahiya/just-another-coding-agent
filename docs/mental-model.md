...
Compaction entries stay durable session metadata, but they now also carry an authoritative compaction checkpoint: `checkpoint_messages` plus `checkpoint_through_run_id`. Resumed runs rebuild from that persisted checkpoint instead of regenerating a synthetic summary boundary on the fly.
