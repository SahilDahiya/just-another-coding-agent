...
- the visible runtime-framing payload now carries current date, timezone, workspace root, shell family, model, thinking setting, and optional effective-capability posture so those changes can stay in the smaller update path instead of forcing a full prefix reset
