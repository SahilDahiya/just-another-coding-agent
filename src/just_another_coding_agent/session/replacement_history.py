...
def _is_real_user_prompt_message(
    ...
    *,
    synthetic_prompt_texts: frozenset[str] | None = None,
) -> bool:
    if not isinstance(message, ModelRequest):
        return False
    for part in message.parts:
        if not isinstance(part, UserPromptPart):
            continue
        if synthetic_prompt_texts is not None:
            content = part.content
            if isinstance(content, str) and content in synthetic_prompt_texts:
                continue
        return True
    return False
...
def _is_anchor_message(
    message: ModelMessage,
    *,
    synthetic_prompt_texts: frozenset[str] | None = None,
) -> bool:
    if _is_real_user_prompt_message(
        message, synthetic_prompt_texts=synthetic_prompt_texts
    ):
        return True
    if is_compaction_summary_message(message):
        return True
...
def build_in_run_truncated_history(
    *,
    ...
    #   via the synthetic_prompt_texts set.
